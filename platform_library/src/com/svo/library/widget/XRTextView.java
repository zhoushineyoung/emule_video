...
public class XRTextView extends TextView {
	...
	public XRTextView(Context context, AttributeSet attrs) {
		...
		textColor = attrs.getAttributeIntValue(namespace, "textColor", Color.WHITE);// 字体颜色
	...
	protected void onDraw(Canvas canvas) {
		...
		for (int i = 0; i < textCharArray.length; i++) {
			...
			if (color) {
				...
				canvas.drawText(textCharArray, i, 1, paddingLeft + drawedWidth, (lineCount + 1) * textSize
						* LineSpacing, paintColor);
			} else {

				canvas.drawText(textCharArray, i, 1, paddingLeft + drawedWidth, (lineCount + 1) * textSize
						* LineSpacing, paint1);
			}
			if (textCharArray[i] > 127 && textCharArray[i] != '、' && textCharArray[i] != '，' && textCharArray[i] != '。'
